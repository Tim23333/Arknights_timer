...
namespace Arknights
{
    internal static class AkSpriteHelper
    {
        ...
        public static List<PortraitSprite> GeneratePortraits(AssetItem asset)
        {
            ...
            if (portraitsData?._sprites == null || portraitsData._sprites.Length == 0)
                return portraits;

            if (portraitsData._atlas?.Texture == null || portraitsData._atlas?.Alpha == null)
                return portraits;

            var atlasTexAsset = Studio.exportableAssets.Find(x => x.m_PathID == portraitsData._atlas.Texture.m_PathID);
            var atlasAlphaAsset = Studio.exportableAssets.Find(x => x.m_PathID == portraitsData._atlas.Alpha.m_PathID);
            if (atlasTexAsset == null || atlasAlphaAsset == null)
                return portraits;

            var atlasTex = (Texture2D)atlasTexAsset.Asset;
            var atlasAlpha = (Texture2D)atlasAlphaAsset.Asset;
