...
一键扫描: 全堆扫描定位敌人列表 (首次/换关卡后需要, 约 3-6 分钟)
...
class PollWorker(QThread):
    ...
    def run(self):
        while not self.isInterruptionRequested():
            ...
            dt = time.time() - t0
            wait = max(0.003, self.interval - dt)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.spin_interval.setRange(0.02, 10.0)
        self.spin_interval.setSingleStep(0.01)
        self.spin_interval.setValue(0.03)
    ...
    def on_snapshot(self, snap):
        # 渲染节流: 轮询可达 50Hz+, 人眼 30fps 足够, 避免 Qt 控件重建成为瓶颈
        now = time.time()
        if snap.get('ok') and now - getattr(self, '_last_render', 0) < 0.033:
            return
    ...
    def _render_table(self, enemies):
        self.table.setRowCount(len(enemies))
        for row, e in enumerate(enemies):
            def item(text, align=Qt.AlignCenter):
                it = QTableWidgetItem(str(text))
                it.setTextAlignment(align)
                return it

            self.table.setItem(row, 0, item(row))
            self.table.setItem(row, 1, item(e.name or e.eid or '?', Qt.AlignLeft | Qt.AlignVCenter))
            self.table.setItem(row, 2, item(e.code or '-'))
            self.table.setItem(row, 3, item(e.eid, Qt.AlignLeft | Qt.AlignVCenter))
            ...
            self.table.setItem(row, 5, item(f'({e.pos_x:.2f}, {e.pos_y:.2f})'))
            self.table.setItem(row, 6, item(int(e.atk)))
            self.table.setItem(row, 7, item(int(e.def_)))
            self.table.setItem(row, 8, item(int(e.res)))
            self.table.setItem(row, 9, item(f'{e.mspd:.2f}'))
            self.table.setItem(row, 10, item(int(e.aspd)))
            st = item('存活' if e.alive else ('退场' if e.finish else '阵亡'))
            if not e.alive:
                st.setForeground(QColor('#888888'))
            self.table.setItem(row, 11, st)
